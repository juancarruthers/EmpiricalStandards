...
function generate_author_deviation_block(checklistItem_id) {
	var deviation_block = generate_question_block_with_yes_no_radio_answers("deviation_block", "deviationRadio", "¿El manuscrito justifica la desviación?", checklistItem_id, 2.4);
...
function generate_one_phase_reviewer_deviation_block(checklistItem_id) {
	...
	// 2nd Question
	var deviation_block = generate_question_block_with_yes_no_radio_answers("deviation_block", "deviationRadio", "¿La desviación es razonable?", checklistItem_id, 2.40);
...
function notify_testers(){
	...
	if(tester){
		...
		if(unrecognized_tags != "")
			alert_msg += "Cuidado — etiqueta/s desconocidas:\n" + unrecognized_tags;
		if(standards_with_no_tags != "")
			alert_msg += "\nCuidado — no hay etiquetas en:\n" + standards_with_no_tags;
		if(standards_with_untagged_attributes != "")
			alert_msg += "\nCuidado — hay atributos sin etiquetar en:\n" + standards_with_untagged_attributes;
		if(alert_msg != "")
		    alert(alert_msg);
	}
}
...
function create_requirements_checklist(){
	...
	var EssentialUL = create_requirements_heading_with_UL("Esencial");

	// create Header for Desirable Requirements with an unordered list
	var DesirableUL = create_requirements_heading_with_UL("Deseable");

	// create Header for Extraordinary Requirements with an unordered list
	var ExtraordinaryUL = create_requirements_heading_with_UL("Extraordinario");
...
function create_for_more_info_part(standard_keys){
	...
	more_info_H2.innerHTML = "Para más información, consultar:";
...
function saveFile(){
	...
	generated_text += "\n" +
		...
		"R = una desviación razonable, aceptable de los estándares\n" +
